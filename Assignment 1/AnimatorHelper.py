...
class AxisSetup:
    ...
    def plot_setup(self, pre_fig=None):
        ...
        self.ax_rod_length = axes([0.84 + slider_pad, 0.75, slider_w, p_top - p_bot - (0.75 - 0.10)],
                                  facecolor=Color.SLIDER_INACTIVE)
        self.ax_bob_mass = axes([0.84 + slider_pad * 2 + slider_w, 0.75, slider_w, p_top - p_bot - (0.75 - 0.10)],
                                facecolor=Color.SLIDER_INACTIVE)
        self.ax_dt = axes([0.84 + slider_pad * 3 + 2 * slider_w, 0.75, slider_w, p_top - p_bot - (0.75 - 0.10)],
                          facecolor=Color.SLIDER_INACTIVE)

        self.ax_calc_method = axes([0.82 + slider_pad, p_bot + 0.29 + button_w, button_w, button_w],
        ...
        self.slider_rod_length = Slider(self.ax_rod_length, "l", 0.5, 4.0, color=Color.SLIDER_ACTIVE,
        ...
        self.slider_bob_mass = Slider(self.ax_bob_mass, "m", 0.1, 10.0, color=Color.SLIDER_ACTIVE,
                                      ...
                                      valstep=0.1, orientation="vertical")
        self.slider_dt = Slider(self.ax_dt, "dt", 0.001, 1, color=Color.SLIDER_ACTIVE, valinit=self.parent.dt,
                                valstep=0.001, orientation="vertical", valfmt="%1.3f")

        self.radio_calc_method = RadioButtons(self.ax_calc_method, ["Euler", "Euler-Cromer", "Runge-Kutta (4)"],
                                              active=self.parent.method_index,
                                              activecolor=Color.RADIO_SELECTED)
        for i in self.radio_calc_method.labels: i.set_fontsize("small")
    ...
    def energy_diff_plot_init(self):
        ...
        self.energy_plot_diff.yaxis.set_label_coords(-0.02, 1.02)
